    ...
    static class CollectionUtil {
        ...
        public static <T extends BaseEntity> boolean hasDuplicates(Collection<T> entities, T targetEntity) {
            return entities.stream().filter(entity ->
                    entity.getUuid() != null && entity.getUuid().equals(targetEntity.getUuid())).count() > 1;
        }
        ...
        public static void swap(List<?> elements, int i, int j) {
            ...
            swapInternal(elements, i, j);
        }
        
        private static <T> void swapInternal(List<T> elements, int i, int j) {
